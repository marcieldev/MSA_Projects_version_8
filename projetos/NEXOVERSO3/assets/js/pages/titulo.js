...
import {escapeHtml,categoryLabel,contentTypeLabel,params,sortedEpisodes} from '../core/utils.js';
...
(async()=>{
  try{
    ...
    el.innerHTML=`<section class="detail"><div><img class="poster" src="${escapeHtml(assetUrl(item.imagem||'assets/img/poster-aster.svg'))}" alt="Pôster de ${escapeHtml(item.titulo)}"></div><div><span class="card-type ${escapeHtml(item.categoria)}">${categoryLabel(item.categoria)}</span><h1>${escapeHtml(item.titulo)}</h1><div class="detail-content-type">${escapeHtml(contentTypeLabel(item.tipo_conteudo))}</div><div class="meta"><span>${escapeHtml(item.ano||'')}</span><span>· ${escapeHtml(item.classificacao||'Livre')}</span><span>· ${(item.generos||[]).map(escapeHtml).join(' · ')}</span></div><p class="detail-copy">${escapeHtml(item.sinopse||'')}</p>${sourceProfile(item)}</div></section><section class="episodes"><div class="section-head"><div><p class="eyebrow">PLAYLIST</p><h2>${item.categoria==='filme'?'Filme completo':'Episódios'}</h2><p class="section-copy">Os episódios podem apontar para vídeos, playlists do YouTube ou outros sites. O NEXOVERSO apenas redireciona para a mídia externa.</p></div></div><div class="episodes-list">${filmList}</div></section>`;
  }catch(e){
