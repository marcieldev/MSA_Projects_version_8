...
export async function catalogo(){const sources=await Promise.all(CATALOG_FILES.map(path=>json(path)));return sources.flatMap(source=>(source.itens||[]).map(item=>({...item,tipo:item.tipo||source.tipo||'especial'})))}
