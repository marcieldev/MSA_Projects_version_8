...
export async function catalogo(){const sources=await Promise.all(CATALOG_FILES.map(path=>json(path)));return sources.flatMap(source=>(source.itens||[]).map(item=>({...item,categoria:item.categoria||source.categoria||'',subcategoria:item.subcategoria??item.tipo_conteudo??''}))) }
